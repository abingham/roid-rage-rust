...
pub fn collision_point(
    ...
    target_velocity: &dyn Velocity,
) -> Option<Point2<f64>> {
    let delta_x = position[0] - target_position.x;
    let delta_y = position[1] - target_position.y;

    let cos_target_bearing = f64::cos(target_velocity.bearing());
    let sin_target_bearing = f64::sin(target_velocity.bearing());
    ...
    solve_quadratic(a, b, c)
        ...
        .map(|dt| {
            let coll_x = dt * target_velocity.speed() * f64::cos(target_velocity.bearing()) + target_position.x;
            let coll_y = dt * target_velocity.speed() * f64::sin(target_velocity.bearing()) + target_position.y;
